...
const Lightfall = ({
  ...
  const programRef = useRef<unknown | null>(null);
  const meshRef = useRef<unknown | null>(null);
  const geometryRef = useRef<unknown | null>(null);
  ...
  useEffect(() => {
    ...
    const uniforms: {
      ...
      uBgColor: Uniform<RGB>;
      uMouseColor: Uniform<RGB>;
      uSpeed: Uniform<number>;
      uStreakCount: Uniform<number>;
      uStreakWidth: Uniform<number>;
      uStreakLength: Uniform<number>;
      uGlow: Uniform<number>;
      uDensity: Uniform<number>;
      uTwinkle: Uniform<number>;
      uZoom: Uniform<number>;
      uBgGlow: Uniform<number>;
      ...
      uSpeed: { value: reducedMotionRef.current ? speed * 0.05 : speed },
      uStreakCount: { value: Math.max(1, Math.min(16, Math.round(streakCount))) },
      uStreakWidth: { value: streakWidth },
      uStreakLength: { value: streakLength },
      uGlow: { value: glow },
      uDensity: { value: density },
      uTwinkle: { value: reducedMotionRef.current ? 0 : twinkle },
      uZoom: { value: zoom },
      uBgGlow: { value: backgroundGlow },
      uOpacity: { value: opacity },
      uMouseEnabled: { value: mouseInteraction && !reducedMotionRef.current ? 1 : 0 },
    ...
    if (mouseInteraction && !reducedMotionRef.current) {
    ...
    const onMotionChange = (event: MediaQueryListEvent) => {
      ...
      uniforms.uSpeed.value = event.matches ? speed * 0.05 : speed;
      uniforms.uTwinkle.value = event.matches ? 0 : twinkle;
      uniforms.uMouseEnabled.value = mouseInteraction && !event.matches ? 1 : 0;
      if (event.matches && pointerListenerAttached) {
        canvas.removeEventListener("pointermove", onPointerMove);
        pointerListenerAttached = false;
      } else if (!event.matches && mouseInteraction && !pointerListenerAttached) {
        canvas.addEventListener("pointermove", onPointerMove);
        pointerListenerAttached = true;
      }
    };
    ...
    return () => {
      ...
      const callIfFn = (obj: unknown, key: "remove" | "destroy") => {
        const candidate = obj as { remove?: () => void; destroy?: () => void } | null;
        if (candidate && typeof candidate[key] === "function") {
          candidate[key]?.call(candidate);
        }
      };
