...
import { CalendarDays, RefreshCw, Shield, Zap } from "lucide-react";
...
export default function SubscriptionStatusCard() {
  ...
  return (
    <div className={`rounded-2xl border bg-white p-6 shadow-sm ${isFounder && data.active ? "border-emerald-200/60" : "border-black/6"}`}>
      <div className="flex items-center gap-2 mb-5">
        <Shield size={14} className={isFounder && data.active ? "text-emerald-600" : "text-gray-400"} />
        <p className="font-bricolage text-xs font-bold text-gray-500 uppercase tracking-widest">Subscription Status</p>
      </div>

      <div className="flex items-center gap-2.5 mb-5">
        <motion.div animate={isFounder && data.active ? { scale: [1, 1.2, 1] } : {}} transition={{ repeat: Infinity, duration: 2 }} className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${isFounder && data.active ? "bg-emerald-500" : "bg-gray-300"}`} />
        <p className={`font-bricolage text-lg font-bold ${isFounder && data.active ? "text-gray-900" : "text-gray-500"}`}>
      ...
      <div className="space-y-3">
        ...
        {isFounder && <InfoRow icon={<RefreshCw size={10} className="text-gray-400" />} title={cycleLabel} subtitle="Billing cycle" />}
        ...
        {isFree && <InfoRow icon={<CalendarDays size={10} className="text-gray-400" />} title="Resets monthly" subtitle="15 analyses per month" />}
      </div>
      ...
      {isFree && (
        <div className="mt-5 pt-4 border-t border-black/5">
          ...
          <p className="font-jakarta text-[10px] text-gray-400 text-center mt-2">500 analyses/month, all engines, PDF exports</p>
        </div>
      )}
