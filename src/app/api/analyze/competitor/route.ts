...
import { analyzeCompetitors } from "@/lib/ai";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await saveAnalysis({ sessionId, engineType: "competitor", inputData: validation.data as unknown as Record<string, unknown>, outputData: result as unknown as Record<string, unknown>, ipHash });
    return NextResponse.json({ success: true, data: result }, { status: 200, headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    console.error("[CompetitorEngine] Analysis failed:", error);
