...
export async function POST(request: NextRequest) {
  ...
  const usageCheck = await checkAnalysisAccess(request, "psychology");
  if (!usageCheck.allowed) return usageCheck.response!;

  try {
    const result = await analyzePsychology(validation.data);
    const recorded = await recordAnalysisUsage(usageCheck.userId!, "psychology", usageCheck.entitlements.monthlyAnalyses);
    ...
    if (usageCheck.entitlements.canSaveHistory) {
      await saveAnalysis({ sessionId, engineType: "psychology", inputData: validation.data as unknown as Record<string, unknown>, outputData: result as unknown as Record<string, unknown>, ipHash, userId: usageCheck.userId ?? undefined });
    }
