...
import { analyzeGrowth } from "@/lib/ai";
import { growthEngineSchema, validateInput } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
  ...
  const validation = validateInput(growthEngineSchema, body);
  if (!validation.success) {
    return NextResponse.json({ success: false, error: "Validation failed", details: validation.errors }, { status: 422 });
  }

  const usageCheck = await checkAnalysisAccess(request, "growth");
  if (!usageCheck.allowed) return usageCheck.response!;

  try {
    const result = await analyzeGrowth(validation.data);
    const recorded = await recordAnalysisUsage(usageCheck.userId!, "growth", usageCheck.entitlements.monthlyAnalyses);
    ...
    if (usageCheck.entitlements.canSaveHistory) {
      await saveAnalysis({ sessionId, engineType: "growth", inputData: validation.data as unknown as Record<string, unknown>, outputData: result as unknown as Record<string, unknown>, ipHash, userId: usageCheck.userId ?? undefined });
    }
