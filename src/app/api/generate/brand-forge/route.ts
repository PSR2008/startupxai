...
export async function POST(request: NextRequest) {
  ...
  const usageCheck = await checkGenerationAccess(request, "brand-forge");
  if (!usageCheck.allowed) return usageCheck.response!;

  try {
    const result = await generateBrand(validation.data);
    const recorded = await recordGenerationUsage(usageCheck.userId!, "brand-forge", usageCheck.plan);
    if (!recorded.inserted) {
      return limitReachedAfterWorkResponse({ feature: "brand-forge", currentUsage: recorded.currentUsage, limit: recorded.limit, plan: usageCheck.plan });
    }
    const ipHash = await hashIp(ip);
    const sessionId = request.headers.get("x-session-id") || `anon_${Date.now()}`;
    if (usageCheck.entitlements.canSaveHistory) {
      await saveAnalysis({ sessionId, engineType: "brand-forge", inputData: validation.data as unknown as Record<string, unknown>, outputData: result as unknown as Record<string, unknown>, ipHash, userId: usageCheck.userId ?? undefined });
    }
